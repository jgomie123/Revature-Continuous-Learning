 ...
 * O(log(n))(log(n) - Logarithmic time: grows slowly; binary search or some sort of variant, balanced binary search tree lookup
...
public class searchingalgorithms {
	...
	public static void main(String[] args) {
		...
		System.out.println(searchLinearly(1, numArray));
		System.out.println(searchBinarily(1, numArray));
	...
	public static int searchBinarily(int numToFind, int[] numArray) {
		...
		while(!isNum) {
			...
			//Take the average of my lower and upper bound so I can find the middle
			int indexToInspect = (lowerBound + upperBound) / 2;
			
			if(numArray[indexToInspect] == numToFind) {
				isNum = true;
			} else if (numArray[indexToInspect] > numToFind) { //if the numToFind is smaller than the midpoint
				upperBound = indexToInspect - 1;
			} else if(numArray[indexToInspect] < numToFind) { //if the numToFind is larger than the midpoint
