...
public class BinaryTree {
	 ...
	 * looking through the same-level children before going deeper into the tree
	 * AKA level-order search.
	 * For this type of search, we'll use a Queue for the implementation, which will
	 * hold the nodes for each level in order
	 * */
	public static void levelOrderTraversal(BinaryTree tree) {
		...
		Queue<BinaryTree> treeList = new LinkedList<>();
		treeList.add(tree);
		
		while(!treeList.isEmpty()) {
			BinaryTree node = treeList.remove();
